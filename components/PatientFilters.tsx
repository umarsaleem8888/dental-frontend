...
export default function PatientFilter({ handleTypeChange }: any) {
  ...
  const [selected, setSelected] = useState(
    localStorage.getItem("patient_filter_type") || "all"
  );

  const [fromDate, setFromDate] = useState(
    localStorage.getItem("patient_filter_from") || ""
  );
  const [toDate, setToDate] = useState(
    localStorage.getItem("patient_filter_to") || ""
  );
  ...
  const persist = (type: string, from = "", to = "") => {
    localStorage.setItem("patient_filter_type", type);
    localStorage.setItem("patient_filter_from", from);
    localStorage.setItem("patient_filter_to", to);
  };
