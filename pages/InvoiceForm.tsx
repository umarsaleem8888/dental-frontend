...
const InvoiceForm: React.FC = () => {
  ...
  const [walletPrev, setWalletPrev] = useState(0); // could come from API
  ...
  // Invoice type status (Updated: handle paidAmount without items/fee)
  ...
  if (invoiceType === 'PreviousDue') {
    ...
    invoiceTypeStatus = 'Payment Only'; // Paid without checkup fee/items
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (prev?.patientId == formData?.patientId
      && prev?.doctorId == formData?.doctorId
      && prev?.paidAmount == formData?.paidAmount
      && prev?.status == formData?.status
      // && prev?.type == formData?.type
      && prev?.walletStatus == walletRemaining.toFixed(2)
      && prev?.invoiceTypeStatus == invoiceTypeStatus
      && prev?.totalAmount == totalAmount
      && prev?.paymentMethod == paymentMethod
      && prev?.notes == formData?.notes
    ) {
      showToast({ text: 'Please change inputs', type: 'info' });
      return;
    }
    ...
    try {
      ...
      if (res) {
        ...
        showToast({ text: 'Invoice Created', type: 'success' });
      ...
      showToast({ text: 'Invoice creation failed', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* SIDEBAR (Left) */}
        {/* <div className="space-y-6 lg:col-span-1 order-first lg:order-none"> */}
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white dark:bg-slate-900 shadow-lg p-6 rounded-3xl border border-slate-200 dark:border-slate-700 space-y-6 transition-all hover:shadow-xl">
            {/* Header */}
            <div className="flex items-center justify-between">
              <p className="text-sm uppercase text-slate-400 font-bold tracking-wider">
                Invoice Summary
              </p>
